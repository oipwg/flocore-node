...
DB.prototype.batch = function(ops, callback) {
  ...
  // Filter out any put operations that do not have a value associated with them
  const filteredOps = ops.filter((op) => {
    if (op.type == 'put' && !op.value) {
      console.log("Filtering out 'put' db operation with no 'value': " + JSON.stringify(op))
      return false
    }
    return true
  })
  ...
  for(var i = 0; i < filteredOps.length; i++) {
    ...
    assert(Buffer.isBuffer(filteredOps[i].key), 'key NOT a buffer as expected.');

    if (filteredOps[i].value) {

      assert(Buffer.isBuffer(filteredOps[i].value), 'value exists but NOT a buffer as expected.');

    } else {
      if (filteredOps[i].type == 'put') console.log(JSON.stringify(filteredOps[i]))
      assert(filteredOps[i].type !== 'put', 'you must provide a value if your type is put!')
    }
  }

  this._store.batch(filteredOps, callback);
