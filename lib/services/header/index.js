...
HeaderService.prototype._adjustHeadersForCheckPointTip = function(callback) {
  ...
  var start = self._encoding.encodeHeaderHeightKey(1);
  ...
  log.info('Header Service: Scanning back to block 1!');
  ...
  stream.on('data', function(data) {
    ...
    // log.info('Header Service: Processing db header at height (' + header.height + ') against tip height (' + self._tip.height + ')')
    ...
    if (header.height > self._tip.height) {
      // log.info('Header Service: Header has a greater height than tip! Removing header from db and rewinding.')
      removalOps.push({
        type: 'del',
        key: data.key
      });
    ...
    if (header.height < self._tip.height) {
      // log.info('Header Service: Header has a lower height than tip. Scanning forward for next! Header: (' + header.height + ') | Tip height: (' + self._tip.height + ')')
      lastDBHeaderHeight = header.height;
    }

    if (header.height === self._tip.height) {
      log.info('Header Service: Matched header to latest stored service tip height! (' + header.height + ')')
  ...
  stream.on('end', function() {
    ...
    // We don't have headers synced to the db.
    if (!self._lastHeader && lastDBHeaderHeight > 0) {
      log.info('Header Service: The last synced header was not in the database. Rewinding to last found synced height: ' + lastDBHeaderHeight)
      self._tip.height = lastDBHeaderHeight;
      return self._adjustHeadersForCheckPointTip(callback);
    }
