...
MempoolService.prototype._getAddressOperations = function(tx, reverse) {
  ...
  for(var i = 0; i < tx.outputs.length; i++) {
    ...
    ops.push({
      ...
      value: action == 'put' ? this._encoding.encodeMempoolAddressKey(address, tx.txid(), i, 0) : undefined
    });
  }
  ...
  for(i = 0; i < tx.inputs.length; i++) {
    ...
    ops.push({
      ...
      value: action == 'put' ? this._encoding.encodeMempoolAddressKey(address, tx.txid(), i, 1) : undefined
    });
  }
...
MempoolService.prototype._onTransaction = function(tx) {
  ...
  self._db.batch(ops.concat(self._getAddressOperations(tx, true)), function(err) {
    if(err) {
      log.error(err);
      self.node.stop();
    }
